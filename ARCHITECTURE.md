...
#### 5.1 客户端层 (Client Layer)

客户端层是用户与平台交互的直接触点，根据不同用户角色的需求，我们设计了多个前端应用。

**1. 门户网站/Web App (面向乘客)**
-   **定位与目标用户**: 这是机场巴士平台主要的线上入口，服务于所有潜在和现有乘客。它既是公司的“数字名片”，也是核心的在线售票渠道。
-   **技术选型与设计考量**: 采用**单页应用（SPA）**架构（Vue.js / React）。设计考量是为用户提供接近原生应用的、流畅无刷新的丰富交互体验。通过组件化开发，可以高效复用UI元素，提升开发效率和可维护性。
-   **核心功能模块**:
    -   **线路与班次查询**: 提供线路地图、站点列表、发车时刻表等信息的查询功能。
    -   **在线购票**: 完整的在线购票流程，包括选座、下单、支付。
    -   **用户中心**: 乘客可以管理个人信息、查询历史订单、处理退票等。
    -   **信息公告**: 发布官方通知、线路变更等信息。

**2. 移动App (面向乘客与司机)**
-   **定位与目标用户**: 为乘客提供更便捷的移动端体验，并作为司机的移动工作台。
-   **技术选型与设计考量**: 采用**跨平台框架（如Uniapp）**。主要的设计考量是**降本增效**，通过一套代码库同时编译生成iOS和Android两端应用，可以显著降低开发和维护成本。对于司机端功能，跨平台框架足以满足其需求。
-   **核心功能模块**:
    -   **乘客端**: 包含Web App的所有核心功能，并额外利用移动设备特性，如提供电子票二维码、推送通知（班次提醒）等。
    -   **司机端**:
        -   **排班查看**: 司机可以清晰地看到自己未来一段时间的排班任务。
        -   **任务执行**: 上报发车、到达状态。
        -   **应急上报**: 在遇到紧急情况时，可以一键上报信息给调度中心。

**3. 自助售票机 (面向乘客)**
-   **定位与目标用户**: 在机场、客运站等线下场景，为不习惯使用手机购票的乘客提供便捷的自助服务。
-   **技术选型与设计考量**: 通常是定制化的**桌面应用或Web应用**，运行在专用的硬件上。设计上需要考虑大屏幕、触摸操作的交互特性，界面必须极致简洁、流程引导清晰，以降低用户的操作门槛。
-   **核心功能模块**: 购票、取票（网购取票）、支付（支持现金、银行卡、扫码）。

**4. 运营后台 (面向内部员工)**
-   **定位与目标用户**: 这是系统的“指挥中心”，服务于调度员、运营管理员、财务人员、安全监控员等所有内部员工。
-   **技术选型与设计考量**: 同样采用**单页应用（SPA）**架构。设计上更侧重于**信息密度和操作效率**，需要提供复杂的数据表格、表单、数据可视化图表等组件，以满足专业化、精细化的管理需求。
-   **核心功能模块**: 覆盖所有微服务提供的管理功能，包括但不限于班次管理、司机排班、车辆监控、财务对账、报表分析等。

#### 5.2 数据存储层 (Data Persistence Layer)

数据是平台的生命线，我们根据不同数据的特性，采用组合式的数据库方案（Polyglot Persistence），为每个场景选择最合适的工具。

**1. 关系型数据库 (MySQL / PostgreSQL)**
-   **定位与设计哲学**: 它是平台核心业务的“真理之源”（Source of Truth）。其设计的核心是**保证数据的高度一致性和完整性**。对于订单、支付、用户信息、排班计划这类绝对不能出错的核心交易数据，利用关系型数据库提供的ACID事务能力是保证业务正确性的基石。
-   **核心用途**:
    -   **用户服务**: 存储结构化的用户信息、角色、权限。
    -   **票务服务**: 存储订单、支付流水等需要强事务保证的数据。
    -   **调度/运营/财务服务**: 存储排班、车辆档案、财务记录等核心业务实体。

**2. 内存数据库 (Redis)**
-   **定位与设计哲学**: 它是整个系统架构的“高速缓存”和“协调中心”。其设计的核心是**极致的性能**。通过将高频访问的数据置于内存中，可以极大地降低对后端关系型数据库的访问压力，显著提升系统的响应速度和吞吐量。
-   **核心用途**:
    -   **分布式缓存**: 缓存用户会话、热门线路信息、已生成的排班表等，加速读取。
    -   **分布式锁**: 在高并发场景下（如抢票锁座），确保操作的原子性。
    -   **简单消息队列**: 用于实现一些轻量级的异步任务或发布/订阅场景。

**3. NoSQL数据库 (MongoDB / InfluxDB)**
-   **定位与设计哲学**: 它是处理海量、非结构化数据的“专用仓库”。其设计哲学是**拥抱多样性，为特定场景优化**。认识到关系型数据库不擅长处理所有类型的数据，我们为GPS轨迹这类高并发写入的时序数据选择了更专业的解决方案，以实现最佳的写入性能和查询效率。
-   **核心用途**:
    -   **GPS服务**: 使用MongoDB或InfluxDB存储海量的车辆GPS轨迹点。这类数据库对时序数据和地理空间查询（如“查找附近5公里的车辆”）有专门的优化。
    -   **日志中心**: 也可用于存储和查询非结构化的应用日志。

#### 5.3 第三方集成 (Third-Party Integration)

-   **定位与设计哲学**: 这一层是平台连接外部生态的“适配器”和“翻译官”。其核心设计哲学是**防腐层（Anti-Corruption Layer）**。外部系统的接口、协议和稳定性是我们无法控制的，通过建立一个专门的防腐层来封装与外部系统的交互，可以有效隔离外部变化对我们核心业务逻辑的冲击，保证了我们内部系统的“纯洁”和稳定。
-   **集成策略**:
    -   **第三方支付 (Payment Gateway)**: 由**票务服务**内的支付适配器负责集成。适配器将封装与支付宝、微信支付等不同渠道的API交互细节（如签名、验签、异步回调处理），对内提供统一的支付接口。
    -   **IC卡系统 (IC Card System)**: 同样由**票务服务**集成。考虑到这类系统可能较为传统，集成方式可能是专线网络下的API调用、共享数据库视图甚至是文件交换。适配器将这些复杂的、异构的集成方式隐藏起来。
    -   **财务收款系统 (Finance System)**: 由**财务服务**负责对接。主要用于处理大客户的线下支付（转账、支票）的对账销账。集成方式可能是定时读取共享文件（如CSV），或是调用财务系统提供的专用对账接口。
    -   **短信服务 (SMS Service)**: 作为一个公共基础服务，会封装成一个独立的客户端库或一个极小的微服务。它将隐藏不同短信供应商（如阿里云、腾讯云）的API差异，对上层服务（如用户服务发送验证码）提供一个统一、简单的 `sendSms` 接口。
